...
export function Footer() {
  ...
  return (
    <footer style={{ background: 'var(--bg)', borderTop: '1px solid var(--border-light)' }}>
          ...
          <div className="col-span-2 md:col-span-1 flex flex-col gap-4">
            <a href="/" className="flex items-center gap-2 w-fit group">
              ...
              <span style={{ fontFamily: 'var(--font-syne)', fontWeight: 700, fontSize: 16 }}>
                <span style={{ color: 'var(--text-1)' }}>tipo</span>
                <span style={{ color: 'var(--gold)' }}>7</span>
              </span>
            </a>
            <p
              ...
              <ul className="flex flex-col gap-2.5">
                {itens.map(link => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      className="t7-hover-text1 text-xs"
                      style={{ fontFamily: 'var(--font-dm-sans)', color: 'var(--text-3)' }}
                    >
                      {link.label}
                    </a>
