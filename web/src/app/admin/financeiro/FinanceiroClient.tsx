...
export function FinanceiroClient({ defaultFeePct, minFeePct, totalConectados, mediaFee }: Props) {
    ...
    try {
      await Promise.all([
        ...
        fetch('/api/admin/settings', {
          method:  'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body:    JSON.stringify({ min_fee_pct: parseFloat(minFee) }),
        }),
      ])
      setSaved(true)
      ...
      <div className="rounded-2xl p-6" style={{ background: '#0d0d0d', border: '1px solid #1a1a1a' }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={handleSalvar} disabled={saving}
            ...
            {saving ? <Loader2 size={14} className="animate-spin" /> : saved ? <><Check size={14} /> Salvo!</> : 'Salvar'}
          </button>
        </div>
      </div>
