...
export default async function ConteudoPage() {
  ...
  // GET /admin/conteudo?key=X é pública e só aceita uma chave por vez — 4
  // chamadas paralelas em vez de ampliar a rota (Fase 7.2, G15).
  const [termosRes, privacidadeRes, lgpdRes, tutorialRes] = await Promise.all([
    ...
    apiFetchServer('/api/admin/conteudo?key=tutorial_evento'),
  ])
  ...
  const tutorial     = tutorialRes.ok    ? (await tutorialRes.json() as { content: string }).content     : ''
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <ConteudoClient termos={termos} privacidade={privacidade} lgpd={lgpd} tutorial={tutorial} />
    </div>
  )
}
