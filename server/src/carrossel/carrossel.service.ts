...
export class CarrosselService {
  ...
  async upload(userId: string, orgId: string | undefined, file: Express.Multer.File | undefined) {
    ...
    const { error: storageErr } = await this.storageService.storage
      .from('carrossel')
    ...
    const {
      data: { publicUrl },
    } = this.storageService.storage.from('carrossel').getPublicUrl(path);
    ...
    try {
      ...
      await this.storageService.storage.from('carrossel').remove([path]);
  ...
  async remover(userId: string, slideId: string) {
    ...
    await this.storageService.storage.from('carrossel').remove([slide.storagePath]);
